...
public class TSAGeoMag
{
	 ...
	 */
	// The model is only valid for its 5-year window [epoch, epoch + 5]. Within it, the linear
	// secular-variation term below extrapolates accurately. Outside it, clamp to the nearest
	// boundary instead of extrapolating unboundedly: before the window use the epoch field, after
	// it (e.g. past 2030 for WMM2025) freeze at the last valid values until newer coefficients are
	// dropped in. This keeps the error bounded rather than diverging.
