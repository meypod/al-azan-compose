...
class OldExportedSettingsSerializationTest {
    ...
    fun parsesOldExportedSettingsJson() {
        val jsonUrl = javaClass.classLoader?.getResource("al_azan_settings.json")
        ...
        assertEquals("masjid_an_nabawi", parsed.settingsStorage.state.savedAdhanAudioEntries[0].let { (it as OldAdhanAudioEntry.OldResourceAdhanAudioEntry).id })
        assertEquals(true, (parsed.settingsStorage.state.savedAdhanAudioEntries[0] as OldAdhanAudioEntry.OldResourceAdhanAudioEntry).internal)
        assertEquals("abdul_basit_abdus_samad", (parsed.settingsStorage.state.savedAdhanAudioEntries[1] as OldAdhanAudioEntry.OldResourceAdhanAudioEntry).id)
        assertEquals("https://github.com/meypod/audio_files/raw/main/adhan/abdul_basit_abdus_samad.mp3", (parsed.settingsStorage.state.savedAdhanAudioEntries[1] as OldAdhanAudioEntry.OldResourceAdhanAudioEntry).remoteUri)
        ...
        assertEquals("masjid_an_nabawi", (defaultAdhan as OldAdhanAudioEntry.OldResourceAdhanAudioEntry).id)
        ...
        assertEquals(null, parsed.calcSettingsStorage.state.roundingMethod)
        ...
        val reminder = parsed.reminderSettingsStorage.state.reminders.first()
        ...
        assertTrue(reminderSound is OldAudioEntry.OldExternalOldAudioEntry || reminderSound is OldAudioEntry.OldResourceOldAudioEntry)
        if (reminderSound is OldAudioEntry.OldExternalOldAudioEntry) {
            assertEquals("default", reminderSound.id)
        } else if (reminderSound is OldAudioEntry.OldResourceOldAudioEntry) {
            assertEquals(1, reminderSound.filepath)
        }
        ...
        assertEquals(false, parsed.settingsStorage.state.deliveredAlarmTimestamps.isEmpty())
        assertEquals(1765204380000L, parsed.settingsStorage.state.deliveredAlarmTimestamps["widget-update-notification"])
    }

    @Test
    fun parsesOldExportedSettingsJson_v2() {
        val jsonUrl = javaClass.classLoader?.getResource("al_azan_settings_2.json")
        ...
        assertTrue(selIsha is OldAdhanAudioEntry.OldExternalAdhanAudioEntry || selIsha is OldAdhanAudioEntry.OldResourceAdhanAudioEntry)
        assertEquals(listOf(Prayer.Midnight, Prayer.Tahajjud), s.hiddenPrayers)
        assertEquals(70, s.adhanVolume)
        assertEquals(listOf(Prayer.Tahajjud, Prayer.Isha, Prayer.Asr, Prayer.Sunset), s.hiddenWidgetPrayers)
        ...
        assertEquals("6dc142c11b83c6bd4b84f826545993b55a97e8d711d6edac59362ae180108ba3", s.calcSettingsHash)
        assertEquals("637e9c131131658a0fbca3542a962e88534643b8c37c940a76a62ce9f5eba28d", s.alarmSettingsHash)
        assertEquals("97d6403543fb9f445c7a2fa11a4c4cd7d09f616d9f0da1eb654352757d0af23d", s.reminderSettingsHash)
        ...
        assertEquals("asr", counters[2].id); assertEquals(2, counters[2].count)
        assertEquals("maghrib", counters[3].id); assertEquals(2, counters[3].count)
        assertEquals("isha", counters[4].id); assertEquals(2, counters[4].count)
        assertEquals("fast", counters[5].id); assertEquals(0, counters[5].count)
        assertEquals("counter_1764365125916", counters[6].id); assertEquals("ذکر", counters[6].label); assertEquals(100, counters[6].lastCount)
        ...
        assertEquals(true, daysMap0[0]); assertEquals(true, daysMap0[1]); assertEquals(true, daysMap0[5]); assertEquals(true, daysMap0[6])
        val r0sound = r0.sound as OldAudioEntry.OldExternalOldAudioEntry
        assertEquals("70", r0sound.id)
        assertTrue(r0sound.filepath?.startsWith("content://") == true)
        ...
        assertTrue(r1.days == null || r1.days is OldPrayerAlarmSettings.Bool || r1.days is OldPrayerAlarmSettings.ByWeekDay)
        val r1sound = r1.sound as OldAudioEntry.OldResourceOldAudioEntry
        assertEquals(null, (r1sound as OldAudioEntry.OldResourceOldAudioEntry).filepath.takeIf { it == null })
        ...
        assertEquals(null, c.roundingMethod)
